...
io.on('connection', (socket) => {
    socket.on('joinRoom', ({ username, room }) => {
        ...
        if (user.isReconnected) {
            // Bem-vindo usuário atual
            socket.emit(
                'message',
                formatMessage(botName, 'Reconectado ao ChatApp!')
            );

            // Transmitir quando um usuário se reconecta
            socket.broadcast
                .to(user.room)
                .emit(
                    'message',
                    formatMessage(
                        botName,
                        `${user.username} reconectou na sala!`
                    )
                );
        } else {
            // Bem-vindo usuário atual
            socket.emit(
                'message',
                formatMessage(botName, 'Bem-vindo(a) ao ChatApp!')
            );
